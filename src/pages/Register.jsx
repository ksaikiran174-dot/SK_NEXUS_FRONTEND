...
function Register({
    ...
    `${import.meta.env.VITE_API_URL}/auth/upload-payment-image`,
    {
      ...
      body: imageFormData
        ...
        if (response.ok) {
          ...
          setSuccess(
            "✓ Restaurant workspace created successfully!"
          );
  ...
  <select
    ...
    required /* Form validation safety wrapper */
  >
    {/* NEW DEFAULT PLACEHOLDER OPTION */}
...
{/* ✅ Only renders the button on screen IF a plan has been selected */}
{formData.plan && (
  <button
    type="button"
    className={`btn-payment ${paymentCompleted ? 'completed' : ''}`}
    onClick={() => setShowPaymentModal(true)}
    /* Disables clicking if payment is already done */
    disabled={paymentCompleted}
  >
    {paymentCompleted ? (
      <>✅ Payment Completed</>
    ) : (
      <>Make Payment ₹{PLAN_PRICES[formData.plan]}</>
    )}
  </button>
)}
              {/* BUTTON */}
  ...
  <div className="payment-modal-overlay">
    <div className="payment-modal" style={{ position: "relative", padding: "30px 20px" }}>
      ...
      {/* 🎯 2. CONDITIONAL SUBMIT LOGIC */}
      <button
        type="button"
        // Button gets disabled if fields are empty
